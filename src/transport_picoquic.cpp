...
#define LOGGER_TRACE(logger, ...) \
    if (logger) SPDLOG_LOGGER_TRACE(logger, __VA_ARGS__)
#define LOGGER_DEBUG(logger, ...) \
    if (logger) SPDLOG_LOGGER_DEBUG(logger, __VA_ARGS__)
#define LOGGER_INFO(logger, ...) \
    if (logger) SPDLOG_LOGGER_INFO(logger, __VA_ARGS__)
#define LOGGER_WARN(logger, ...) \
    if (logger) SPDLOG_LOGGER_WARN(logger, __VA_ARGS__)
#define LOGGER_ERROR(logger, ...) \
    if (logger) SPDLOG_LOGGER_ERROR(logger, __VA_ARGS__)
#define LOGGER_CRITICAL(logger, ...) \
    if (logger) SPDLOG_LOGGER_CRITICAL(logger, __VA_ARGS__)
...
namespace {
     ...
     */
    constexpr DataContextId MakeDatagramStreamId(bool is_server, bool is_unidirectional) { return 0; }
}  // namespace
...
int PqEventCb(picoquic_cnx_t* pq_cnx, uint64_t stream_id, uint8_t* bytes, size_t length,
    ...
    switch (fin_or_event) {
        ...
        case picoquic_callback_stream_reset: {
            ...
            LOGGER_DEBUG(transport->logger, "Received RESET stream; conn_id: {0} data_ctx_id: {1} stream_id: {2}",
                         data_ctx->conn_id, data_ctx->data_ctx_id, stream_id);
            break;
        }
        ...
        case picoquic_callback_pacing_changed: {
            ...
            LOGGER_INFO(transport->logger,
                        "Pacing rate changed; conn_id: {0} rate Kbps: {1} cwin_bytes: {2} rtt_us: {3} rate Kbps: {4} "
                        "cwin_bytes: "
                        "{5} rtt_us: {6} rtt_max: {7} rtt_sample: {8} lost_pkts: {9} bytes_in_transit: {10} "
                        "recv_rate_Kbps: {11}",
                        conn_id, stream_id * 8 / 1000, cwin_bytes, rtt_us, path_quality.pacing_rate * 8 / 1000,
                        path_quality.cwin, path_quality.rtt, path_quality.rtt_max, path_quality.rtt_sample,
                        path_quality.lost, path_quality.bytes_in_transit,
    ...
    if (transport == NULL) {
        ...
        switch (cb_mode) {
            case picoquic_packet_loop_ready: {
                ...
                if (transport->is_server_mode) transport->SetStatus(TransportStatus::kReady);
            ...
            case picoquic_packet_loop_time_check: {
                ...
                if (transport->Status() == TransportStatus::kShutdown) {
                    ...
                    while (close_cnx != NULL) {
                        LOGGER_INFO(transport->logger, "Closing connection id {0}",
                                    reinterpret_cast<uint64_t>(close_cnx));
...
TransportStatus PicoQuicTransport::Status() const { return transportStatus_; }

TransportConnId PicoQuicTransport::Start(std::shared_ptr<SafeQueue<MetricsConnSample>> metrics_conn_samples,
...
bool PicoQuicTransport::GetPeerAddrInfo(const TransportConnId& conn_id, sockaddr_storage* addr) {
...
TransportError PicoQuicTransport::Enqueue(const TransportConnId& conn_id, const DataContextId& data_ctx_id,
                                          std::vector<uint8_t>&& bytes, std::vector<MethodTraceItem>&& trace,
                                          const uint8_t priority, const uint32_t ttl_ms,
                                          [[maybe_unused]] const uint32_t delay_ms, const EnqueueFlags flags) {
    if (bytes.empty()) {
...
std::shared_ptr<StreamBuffer<uint8_t>> PicoQuicTransport::GetStreamBuffer(TransportConnId conn_id, uint64_t stream_id) {
    std::lock_guard<std::mutex> _(state_mutex_);
...
std::optional<std::vector<uint8_t>> PicoQuicTransport::Dequeue(
    TransportConnId conn_id, [[maybe_unused]] std::optional<DataContextId> data_ctx_id) {
    std::lock_guard<std::mutex> _(state_mutex_);
...
DataContextId PicoQuicTransport::CreateDataContext(const TransportConnId conn_id, bool use_reliable_transport,
    ...
    if (is_new) {
        ...
        data_ctx_it->second.tx_data = std::make_unique<PriorityQueue<ConnData>>(
            tconfig_.time_queue_max_duration, tconfig_.time_queue_bucket_interval, tick_service_,
...
void PicoQuicTransport::Close(const TransportConnId& conn_id, uint64_t app_reason_code) {
    std::lock_guard<std::mutex> _(state_mutex_);

    const auto conn_it = conn_context_.find(conn_id);

    if (conn_it == conn_context_.end()) return;
...
void PicoQuicTransport::SetDataCtxPriority(const TransportConnId conn_id, DataContextId data_ctx_id, uint8_t priority) {
    ...
    if (data_ctx_it == conn_it->second.active_data_contexts.end()) return;

    LOGGER_DEBUG(logger,
                 "Set data context priority to {0}  conn_id: {1} data_ctx_id: {2}" << static_cast<int>(priority),
                 conn_id, data_ctx_id);

    data_ctx_it->second.priority = priority;
}

void PicoQuicTransport::SetStreamIdDataCtxId(const TransportConnId conn_id, DataContextId data_ctx_id,
                                             uint64_t stream_id) {
    std::lock_guard<std::mutex> _(state_mutex_);
    ...
    LOGGER_DEBUG(logger, "Set data context to stream conn_id: {0} data_ctx_id: {1} stream_id: {2}", conn_id,
                 data_ctx_id, stream_id);
...
PicoQuicTransport::ConnectionContext& PicoQuicTransport::CreateConnContext(picoquic_cnx_t* pq_cnx) {
    ...
    switch (addr->sa_family) {
        case AF_INET:
            (void)inet_ntop(AF_INET, &reinterpret_cast<struct sockaddr_in*>(addr)->sin_addr,
                            ...
                            conn_ctx.peer_addr_text, sizeof(conn_ctx.peer_addr_text));
    ...
    if (is_new) {
        ...
        conn_ctx.dgram_tx_data = std::make_unique<PriorityQueue<ConnData>>(
            tconfig_.time_queue_max_duration, tconfig_.time_queue_bucket_interval, tick_service_,
            tconfig_.time_queue_init_queue_size);
    }
...
PicoQuicTransport::PicoQuicTransport(const TransportRemote& server, const TransportConfig& tcfg,
                                     TransportDelegate& delegate, bool is_server_mode,
...
void PicoQuicTransport::SetStatus(TransportStatus status) { transportStatus_ = status; }

PicoQuicTransport::DataContext* PicoQuicTransport::CreateDataContextBiDirRecv(TransportConnId conn_id,
                                                                              uint64_t stream_id) {
    std::lock_guard<std::mutex> _(state_mutex_);
    ...
    if (is_new) {
        ...
        LOGGER_INFO(logger, "Created new bidir data context conn_id: {0} data_ctx_id: {1} stream_id: {2}", conn_id,
                    data_ctx_it->second.data_ctx_id, stream_id);
...
void PicoQuicTransport::SendNextDatagram(ConnectionContext* conn_ctx, uint8_t* bytes_ctx, size_t max_len) {
    ...
    if (out_data.has_value) {
        ...
        if (data_ctx_it == conn_ctx->active_data_contexts.end()) {
            LOGGER_DEBUG(logger, "send_next_dgram has no data context conn_id: {0} data len: {1} dropping",
                         conn_ctx->conn_id, out_data.value.data.size());
        ...
        if (out_data.value.data.size() == 0) {
            LOGGER_ERROR(logger, "conn_id: {0} data_ctx_id: {1} priority: {2} has ZERO data size",
                         data_ctx_it->second.conn_id, data_ctx_it->second.data_ctx_id,
        ...
        if (out_data.value.data.size() <= max_len) {
            ...
            buf = picoquic_provide_datagram_buffer_ex(
                bytes_ctx, out_data.value.data.size(),
...
void PicoQuicTransport::SendStreamBytes(DataContext* data_ctx, uint8_t* bytes_ctx, size_t max_len) {
    if (bytes_ctx == NULL) {
        return;
    }
    ...
    switch (data_ctx->stream_action) {
        ...
        case DataContext::StreamAction::kReplaceStreamUseReset: {
            ...
            LOGGER_DEBUG(logger,
                         ...
                         data_ctx->metrics.tx_buffer_drops, data_ctx->metrics.tx_queue_discards);
        ...
        case DataContext::StreamAction::kReplaceStreamUseFin: {
            ...
            LOGGER_INFO(logger, "Replacing stream using FIN; conn_id: {0} existing_stream: {1}", data_ctx->conn_id,
    ...
    if (data_ctx->stream_tx_object == nullptr) {
        ...
        if (obj.has_value) {
            if (obj.value.data.size() == 0) {
                LOGGER_ERROR(logger, "conn_id: {0} data_ctx_id: {1} priority: {2} stream has ZERO data size",
                             data_ctx->conn_id, data_ctx->data_ctx_id, static_cast<int>(data_ctx->priority));
                return;
            }
    ...
    if (!is_still_active && !data_ctx->tx_data->Empty()) is_still_active = 1;
    ...
    if (buf == NULL) {
        ...
        LOGGER_ERROR(logger, "conn_id: {0} data_ctx_id: {1} priority: {2} unable to allocate pq buffer size: {3}",
                     data_ctx->conn_id, data_ctx->data_ctx_id, static_cast<int>(data_ctx->priority), data_len);
        return;
    }
...
void PicoQuicTransport::OnConnectionStatus(const TransportConnId conn_id, const TransportStatus status) {
...
void PicoQuicTransport::OnNewConnection(const TransportConnId conn_id) {
    auto conn_ctx = GetConnContext(conn_id);
    if (!conn_ctx) return;

    LOGGER_INFO(logger, "New Connection {0} port: {1} conn_id: {2}", conn_ctx->peer_addr_text, conn_ctx->peer_port,
                conn_id);
    ...
    TransportRemote remote{
        .host_or_ip = conn_ctx->peer_addr_text, .port = conn_ctx->peer_port, .proto = TransportProtocol::kQuic};
...
void PicoQuicTransport::OnRecvDatagram(ConnectionContext* conn_ctx, uint8_t* bytes, size_t length) {
    if (length == 0) {
...
void PicoQuicTransport::OnRecvStreamBytes(ConnectionContext* conn_ctx, DataContext* data_ctx, uint64_t stream_id,
    ...
    if (rx_buf_it == conn_ctx->rx_stream_buffer.end()) {
        ...
        LOGGER_DEBUG(logger, "Adding received conn_id: {0} stream_id: {1} into RX buffer", conn_ctx->conn_id,
                     stream_id);
...
void PicoQuicTransport::CheckConnsForCongestion() {
    ...
    for (auto& [conn_id, conn_ctx] : conn_context_) {
        ...
        for (auto& [data_ctx_id, data_ctx] : conn_ctx.active_data_contexts) {
            ...
            if (data_ctx.priority >= 2 &&
                ...
                LOGGER_WARN(logger,
                            ...
                            data_ctx.conn_id, data_ctx.data_ctx_id, data_ctx.tx_data->Size(),
        ...
        if (cwin_congested_count && conn_ctx.pq_cnx->nb_retransmission_total - conn_ctx.metrics.tx_retransmits > 2) {
            LOGGER_INFO(logger, "CC: remote: {0} port: {1} conn_id: {2} retransmits increased, delta: {3} total: {4}",
                        conn_ctx.peer_addr_text, conn_ctx.peer_port, conn_id,
        ...
        if (congested_count) {
            ...
            LOGGER_WARN(logger,
                        ...
                        conn_id, congested_count, conn_ctx.metrics.tx_retransmits, conn_ctx.metrics.cwin_congested);
            ...
            if (tconfig_.use_reset_wait_strategy && reset_wait_data_ctx_id > 0) {
                ...
                LOGGER_INFO(logger, "CC: conn_id: {0} setting reset and wait to data_ctx_id: {1} priority: {2}",
                            conn_id, reset_wait_data_ctx_id, static_cast<int>(data_ctx.priority));
            ...
            if (conn_ctx.not_congested_gauge > 4) {
                ...
                LOGGER_INFO(logger, "CC: conn_id: {0} congested_count: {1} is no longer congested.", conn_id,
                            congested_count);
            } else {
...
TransportConnId PicoQuicTransport::CreateClient() {
    ...
    picoquic_cnx_t* cnx =
        picoquic_create_cnx(quic_ctx_, picoquic_null_connection_id, picoquic_null_connection_id,
                            reinterpret_cast<struct sockaddr*>(&server_address), current_time, 0, sni, config_.alpn, 1);
...
void PicoQuicTransport::CreateStream(ConnectionContext& conn_ctx, DataContext* data_ctx) {
    ...
    LOGGER_DEBUG(logger, "conn_id: {0} data_ctx_id: {1} create new stream with stream_id: {2}" << conn_ctx.conn_id,
                 data_ctx->data_ctx_id, conn_ctx.last_stream_id);
...
void PicoQuicTransport::CloseStream(ConnectionContext& conn_ctx, DataContext* data_ctx, const bool send_reset) {
    ...
    LOGGER_DEBUG(logger, "conn_id: {0} data_ctx_id: {1} closing stream stream_id: {2}", conn_ctx.conn_id,
    ...
    if (send_reset) {
        ...
        LOGGER_INFO(logger, "Sending FIN for stream_id: {0} conn_id: {1}", *data_ctx->current_stream_id,
                    conn_ctx.conn_id);
...
void PicoQuicTransport::MarkStreamActive(const TransportConnId conn_id, const DataContextId data_ctx_id) {
    ...
    picoquic_mark_active_stream(conn_it->second.pq_cnx, *data_ctx_it->second.current_stream_id, 1,
                                &data_ctx_it->second);
}
