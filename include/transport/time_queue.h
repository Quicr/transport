...
* current bucket index advance distance.
    ...
    * @param interval The interval at which ticks should update.
       ...
       void get_ticks(const duration_t& interval, timer_context& ctx) override
       {
           ...
           ctx.ticks = _ticks / increment; // Get ticks in the requested tick increment
...
* @tparam Duration_t   The duration type to check for.
    ...
    *
    * @param duration  Duration of the queue. Value must be > 0, != interval, and
    * a multiple of interval.
    * @param interval  Interval size of each bucket. Value must be > 0 and !=
    * duration
    * @param timer     Shared pointer to timer service
    *
    * @throws std::invalid_argument If the duration or interval do not meet
    * requirements.
        */
    ...
    * @brief Construct a time_queue with defaults or supplied parameters
    *
    * @param duration              Duration of the queue. Value must be > 0, != interval, and a multiple of interval.
    ...
    * requirements.
        */
       time_queue(size_t duration, size_t interval,
                  std::shared_ptr<queue_timer_service> timer, size_t initial_queue_size)
    ...
    * @param ttl   The time to live of the value, according to Duration_t.
       ...
       void push(const T& value, size_t ttl)
       {
           ...
           internal_push(value, future_bucket_index, expiry_tick);
       }
    ...
    * @param value The value to push onto the queue.
       ...
       void push(const T& value)
       {
           ...
           const auto future_bucket_index =
             (_bucket_index + (_duration / _interval) - 1) % _total_buckets;
    ...
    * @returns Current tick value at time of advance
       ...
       inline tick_type advance()
       {
           ...
           if (_timer_ctx.delta >= _total_buckets) {
               _buckets.clear();
               _buckets.resize(_total_buckets);
    ...
    * then emplaces the location info into the queue.
     ...
     private:
       ...
       /// The total amount of buckets. Value is calculated based on duration /
       /// intreval.
