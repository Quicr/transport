...
class Span {
   ...
   public:
    ...
    constexpr Span() noexcept : _ptr(nullptr), _extent(0) {}

    template <typename It>
    constexpr Span(It first, size_type count) : _ptr(std::addressof(*first)), _extent(count) {
    ...
    constexpr Span(It first, End last) : _ptr(std::addressof(*first)), _extent(static_cast<size_type>(last - first)) {
        if constexpr (extent != dynamic_extent) {
    ...
    constexpr Span(element_type (&arr)[N]) noexcept : Span(static_cast<pointer>(arr), N) {}

    template <class U, size_type N, typename std::enable_if_t<std::is_convertible_v<U, T>, bool> = true>
    constexpr Span(std::array<U, N>& arr) noexcept : Span(static_cast<pointer>(arr.data()), N) {}

    template <class U, size_type N, typename std::enable_if_t<std::is_convertible_v<U, T>, bool> = true>
    constexpr Span(const std::array<U, N>& arr) noexcept : Span(static_cast<pointer>(arr.data()), N) {}
    ...
    template <class U, size_type N,
              typename std::enable_if_t<(Extent == dynamic_extent || N == dynamic_extent || Extent == N) &&
                                        ...
                                        bool> = true>
    constexpr Span(const Span<U, N>& other) noexcept : _ptr(other.data()), _extent(other.size()) {}
