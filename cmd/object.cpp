...
void Object::process(TransportConnId conn_id, std::optional<DataContextId> data_ctx_id, std::vector<uint8_t>& obj) {
    ...
    if (prev_msg_num && ((*msg_num - prev_msg_num) > 1 || (*msg_num - prev_msg_num) < 0)) {
        SPDLOG_LOGGER_INFO(logger,
                           ...
                           "prev_num: {6} ({7})",
                           conn_id, (data_ctx_id ? *data_ctx_id : 0), *msg_len, obj.size(), msgcount, *msg_num,
                           prev_msg_num, *msg_num - prev_msg_num);
    }
    ...
    if (*msg_num % 2000 == 0) {
        SPDLOG_LOGGER_INFO(
            ...
            conn_id, (data_ctx_id ? *data_ctx_id : 0), *msg_len, obj.size(), msgcount, *msg_num, prev_msg_num,
